...
internal interface IReceiveShipmentHandler : IHandler
{
	Task<Result<Success>> HandleAsync(string shipmentNumber, CancellationToken cancellationToken);
}

internal sealed class ReceiveShipmentHandler(
	ShipmentsDbContext context,
	ILogger<ReceiveShipmentHandler> logger)
	: IReceiveShipmentHandler
{
